...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty(trim($_POST["username"]))) {
    ...
    if ($stmt = mysqli_prepare($CMeritBadges->getDbConn(), $sql)) {
      ...
      error_log($strErr);
      $CMeritBadges::function_alert("Internal Error, this has been reported.");
      $CMeritBadges->GotoURL('./index.php');
      exit();
    }
  }
  ...
  if (empty($username_err) && empty($password_err) && empty($confirm_password_err)) {
    ...
    if ($stmt = mysqli_prepare($con, $sql)) {
      ...
      if (mysqli_stmt_execute($stmt)) {
        ...
        $CMeritBadges->function_alert($msg);
  ...
  if (
    ...
    if ($stmt = mysqli_prepare($CMeritBadges->getDbConn(), $sql)) {
      ...
      if (mysqli_stmt_execute($stmt)) {
        ...
        CMeritBadges::function_alert($msg);
        $str = sprintf(
          "New GOAT registration, on %s - User: %s - Password: %s \n",
          Date('Y-m-d H:i:s'),
          $param_username,
          $param_password
        );
        error_log($str, 1, "richard.hall@centennialdistrict.co");
        CMeritBadges::GotoURL("./logon.php");
      } else {
        echo "Oops! Something went wrong. Please try again later.";
      }
  ...
  // Close connection
  mysqli_close($CMeritBadges->getDbConn());
}
?>
