...
class AdultLeaders
{
  ...
  {
    $Trained = null;
    $qryTrained = "SELECT * FROM trainedleaders WHERE First_Name='$FName' AND Last_Name='$LName' AND Position='$Position' ";
    ...
    if($result_trained){
      ...
      // Report error
      error_log("Error: IsTrained() :" . $qryTrained);
      $Trained = "Unk";
    }
   ...
   * 
   *****************************************************************************/
  public static function FindMemberUnit($data)
  {
    $FName = $data[1];
    $LName = $data[2];
    $Unit1Type = "Troop";
    ...
    if ($result_units) {
      ...
      if (is_array($Units)) {
        if (sizeof($Units) < 1) {
          // See if there is a unit in the troopnos
          $Units[0] = (count($pieces) >= 1) ? $pieces[0] : "None";
          $Units[1] = (count($pieces) >= 2) ? $pieces[1] : "None";" ";
        } else if (sizeof($Units) < 2) {
          $Units[1] = (count($pieces) >= 1) ? $pieces[0] : "None";;
        }
      } else {
        $Units[0] = "NA";
        $Units[1] = "NA ";
      ...
      // Report Error
      error_log("Error: FindMemberUnit(): " .  $qryUnits);
    }
      ...
      public static function UpdateFunctionalRole($fileName)
      {
        ...
        if (($handle = fopen($filePath, "r")) !== FALSE) {
          while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            ...
            $Unit = self::formatUnitNumber($data[$col_displayname], null);
            //Search data and find member
            $sqlFind = "SELECT * FROM trainedleaders WHERE First_Name='$data[$col_firstname]' AND  Last_Name='$data[$col_lastname]' AND Unit='$Unit' ";
            $query = self::doQuery($sqlFind);
            switch (mysqli_num_rows($query)) {
              case  0:
                ...
                break;
              case 1:
                $sqlUpdate = "UPDATE `trainedleaders` SET `FunctionalRole`='$data[$col_positionname]' 
					WHERE First_Name='$data[$col_firstname]' AND  Last_Name='$data[$col_lastname]'  AND Unit='$Unit'";
                $query = self::doQuery($sqlUpdate);
                if ($query)
                  $Updated++;
                else
                  $Error++;
                // Update the Function role
                break;
              case 2:
              case 3:
              case 4:
