<?php
	...
	if(isset($_POST['search']) && strlen($_POST['search']) !== 0)
	{
		...
		$_SESSION['condition'] = " title LIKE '%".AFFC5Calendar::db_escape($_POST['search'])."%' OR description LIKE '%".AFFC5Calendar::db_escape($_POST['search'])."%'";
	...
	if(isset($_POST['filter']) && !strlen($_POST['filter']) !== 0)
	{
		...
		if($filter == 'all-fields')
		{
			...
			$_SESSION['condition'] = "category = '".AFFC5Calendar::db_escape($filter)."'";	
		}
		$calendar = new AFFC5Calendar(DB_HOST, DB_USERNAME, DB_PASSWORD, DATABASE, TABLE, $_SESSION['condition'], $cf);
	} elseif(isset($_SESSION['condition']) && strlen($_SESSION['condition']) !== 0) {
		$calendar = new AFFC5Calendar(DB_HOST, DB_USERNAME, DB_PASSWORD, DATABASE, TABLE, $_SESSION['condition'], $cf);
	} else {
		$calendar = new AFFC5Calendar(DB_HOST, DB_USERNAME, DB_PASSWORD, DATABASE, TABLE, false, $cf);	 
	}
