...
class ICal
{
	...
	/** 
     * Return Unix timestamp from ical date time format - Converted to timezone 
     * 
     * @param {string} $icalDate A Date in the format YYYYMMDD[T]HHMMSS[Z] or
     *                           YYYYMMDD[T]HHMMSS
     *
     * @return {int} 
     */ 
	public function iCalConvertDate($icalDate, $tmz)
	{
		$icalDate = str_replace('T', '', $icalDate); 
		$icalDate = str_replace('Z', '', $icalDate); 
		...
		// Create a DateTime object with the provided date in UTC timezone
		$dateTime = new DateTime("$date[1]-$date[2]-$date[3] $date[4]:$date[5]:$date[6]", new DateTimeZone('UTC'));

		// Convert the DateTime object to the desired timezone
		$dateTime->setTimezone(new DateTimeZone($tmz));
