					...
					try {
						...
						if ($TotalPacks > 0) {
							echo '<div id="loadingOverlay" style="display: none; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 1000; display: flex; justify-content: center; align-items: center;">' .
								...
								'</div>' .
								'<table id="packsMeetingGoalTable" class="table table-striped"><thead><tr>' .
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		console.log('Starting DataTable initialization for packsMeetingGoalTable');

		// Show custom loading overlay
		$('#loadingOverlay').show();
		console.log('Loading overlay shown');

		$('#packsMeetingGoalTable').DataTable({
			dom: 'Bfrtip',
			buttons: [{
					extend: 'copy',
				...
				},
				{
					extend: 'csv',
					className: 'btn btn-primary btn-sm d-print-none mt-2',
					filename: 'Centennial District Packs Meeting Goal'
				},
				{
					extend: 'excel',
					className: 'btn btn-primary btn-sm d-print-none mt-2',
