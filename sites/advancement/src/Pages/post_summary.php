...
load_class(BASE_PATH . '/src/Classes/CPost.php');

$CPost = CPost::getInstance();

try {
  $SelYear = isset($_SESSION['year']) ? $_SESSION['year'] : date("Y");
  $CPost->SetYear($SelYear);

  $Totals = $CPost->GetTotals();
  $NumofPosts = $CPost->GetNumofPosts();
} catch (Exception $e) {
  $_SESSION['feedback'] = ['type' => 'danger', 'message' => 'Error loading post data: ' . $e->getMessage()];
...
<sort_options>
  <div class="px-lg-5">
    <div class="row">
      <div class="col-2">
        <form action="index.php?page=post-summary" method="POST">
    ...
    <div class="row">
      <div class="col-12">
        <div class="py-5">
          <?php
          try {
            $sql = sprintf("SELECT * FROM adv_post WHERE Date=%d ORDER BY Unit ASC", $CPost->GetYear());
            $result = mysqli_query($CPost->getDbConn(), $sql);
            if ($result) {
              ...
              if ($rowcount > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                  $UnitYouth = $CPost->GetUnitTotalYouth($row['Unit'], $row['Youth'], $row['Date']);
                ...
                echo "<p>No Post data available for $SelYear.</p>";
              }
              mysqli_free_result($result);
            } else {
              throw new Exception("Database query failed: " . mysqli_error($CPost->getDbConn()));
            }
            echo "<p style='text-align: center;'>Number of Posts in District: $NumofPosts</p>";
            echo "<p style='text-align: center;'>Data last updated: " . htmlspecialchars($CPost->GetLastUpdated("adv_post")) . "</p>";
          } catch (Exception $e) {
            $_SESSION['feedback'] = ['type' => 'danger', 'message' => 'Error displaying post data: ' . $e->getMessage()];
            error_log("post_summary.php - Error: " . $e->getMessage(), 0);
          }
          ?>
        </div>
      </div>
...
<script type="text/javascript">
  ...
  function drawBarChart() {
    ...
    var options = {
      chart: {
        title: 'District wide Post Awards Data',
  ...
  $(document).ready(function() {
    console.log('Starting DataTable initialization for postTable');
    ...
    $('#crewTable').DataTable({
      ...
      buttons: [{
        ...
        },
        {
          extend: 'csv',
          className: 'btn btn-primary btn-sm d-print-none mt-2',
          filename: 'Centennial District Post Summary'
        },
        {
          extend: 'excel',
          className: 'btn btn-primary btn-sm d-print-none mt-2',
