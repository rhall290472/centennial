...
try {
  ...
  // Get total number of packs in district
  ...
  error_log("PacksBelowGoal.php - Error: " . $e->getMessage(), 0);
  $TotalPacks = 0;
}
?>
...
<sort_options>
  <div class="px-lg-5">
    <div class="row">
      <div class="col-2">
        <form action="index.php?page=packs_below_goal" method="POST">
          ...
          try {
            ...
            if ($TotalPacks > 0) {
              echo '<div id="loadingOverlay" style="display: none; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 1000; display: flex; justify-content: center; align-items: center;">' .
                ...
                '</div>' .
                '<table id="packsBelowGoalTable" class="table table-striped"><thead><tr>' .
...
<script type="text/javascript">
  ...
  function drawChart() {
    var data = google.visualization.arrayToDataTable([
      ...
      try {
        ...
        echo "['Packs above goal', 0],['Packs below goal', 0]";
      }
      ?>
    ]);
  ...
  $(document).ready(function() {
    console.log('Starting DataTable initialization for packsBelowGoalTable');

    // Show custom loading overlay
    $('#loadingOverlay').show();
    console.log('Loading overlay shown');

    $('#packsBelowGoalTable').DataTable({
      dom: 'Bfrtip',
      buttons: [{
          extend: 'copy',
        ...
        },
        {
          extend: 'csv',
          className: 'btn btn-primary btn-sm d-print-none mt-2',
          filename: 'Centennial District Packs Below Goal'
        },
        {
          extend: 'excel',
          className: 'btn btn-primary btn-sm d-print-none mt-2',
