...
<head>
  <meta name="description" content="ReportAwardedYear.php">
</head>
...
<body>
  ...
  <center>
    <h4><?php echo "Nominees for " . $cDistrictAwards->GetYear() ?> </h4>
    <table class="fixed_header">
      ...
      if ($year == "")
        $queryNominees = "SELECT * FROM `district_awards` WHERE `Status`='1' AND (`IsDeleted` IS NULL || `IsDeleted` <>'1') ORDER BY `Award`, `Year` DESC";
      else
        $queryNominees = "SELECT * FROM `district_awards` WHERE `Year`='$year' AND `Status`='1' AND (`IsDeleted` IS NULL || `IsDeleted` <>'1') ORDER BY `Award`, `Unit`, `LastName`";
